...
// example code that sets up windowed access to an MDBM, and then performs
// store/fetch/delete using exclusive locks.
// Use Windowed mode to access very large MDBMs that cannot be mapped into memory in their
// entirety.
...
static size_t   WindowSize = 0;   // No windowed access by default
...
{
    string fileName = "/tmp/test_window.mdbm";
     ...
     char *argv[])
{
    ...
    srandom(587);  // An arbitrary prime
    MDBM *mdbm = openDb(PageSize, InitialDbSize, LargeObjects);

    // Set up Windowed access to the MDBM
    if (WindowSize) {
